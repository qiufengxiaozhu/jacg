...
// 添加选项
        ...
        $(document).on("click", "#test-save-btn", function () {
           ...
           var choiceText04= $("#choiceText04").val();
            ...
            // 添加题目 并且更新题目状态
            zudp.ajax("/api/wxoption/addOption").post(objJson).then(function (value) {
    ...
    function getAllType(obj) {
        ...
        if (str != "") {
            // 查询单个
            zudp.ajax("/api/wxquestion/getOneType?id=" + idstr).get().then(function (value) {
                typeId = value.questionType;
            });
        }
        // 下拉的id -->categor
        $("#category").find("option").remove();
        zudp.ajax("/api/wxquestion/getAllType").get().then(function (value) {
