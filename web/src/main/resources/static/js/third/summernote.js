...
var dialog = renderer.create('<div class="modal imageModal" aria-hidden="false" tabindex="-1"/>', function ($node, options) {
    if (options.fade) {
        $node.addClass('fade');
    }
    $node.html([
        ...
        (options.title
            ? '    <div class="modal-header">' +
                '      <button type="button" class="close" onclick="closeModal(this)" aria-label="Close"><span aria-hidden="true">&times;</span></button>' +
...
function closeModal(a) {
    //$(a).parents(".imageModal").modal("hide");
    var par=$(a).parents(".modal");
    for(var k=0;k<par.length;k++){
        if($(par[k]).hasClass("imageModal")){
            $(par[k]).modal("hide");
        }else{
            $(par[k]).focus();
