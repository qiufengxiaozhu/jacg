...
$("#addItem").click(function () {
    if ($("#org_form").valid()) {
        ...
        if ($("#addItem").val() == 1) {
            var url = "/api/group/groupInsert?parentId=" + checkId + "&isBro=false";
            if ($("#addBroBtn").attr("isBro") == "true") {
                url = "/api/group/groupInsert?parentId=" + checkId + "&isBro=true"
            }
            zudp.ajax(url)
                .post(data)
                .then(function (value) {
            ...
            zudp.ajax("/api/group/groupUpdate/" + recordId)
                .put(data)
                .then(function (value) {
                    zudp.plugin.dialog("success").alert("操作成功！", "提示");
                    findList();
...
$(document).ready(function () {
    ...
    $("#updateBtn").click(function () {
        ...
        zudp.ajax("/api/group/" + checkId)
            .get()
            .then(function (value) {
                $("#org_no").attr("record_id", value.id);
                $("#org_no").val(value.no);
                $("#org_name").val(value.treeName);
    ...
    $("#deleteBtn").click(function () {
        swalFunction("是否确认删除该节点以及其所有子节点", function () {
            zudp.ajax("/api/group/cascade/" + checkId)
                .get()
                .then(function (value) {
                    if (value) {
                        findList();
    ...
    $("#revoke").click(function () {
        zudp.ajax("/api/group/hasChild/" + checkId)
            .get()
            .synch()
            .then(function (value) {
                if (value == "true") {
                    swalFunction("该节点有子组织，是否要级联撤销", function () {
                        revokeNode();
                    }, "确认撤销");
                } else {
                    swalFunction("是否撤销该组织", function () {
                        revokeNode();
                    }, "撤销")
                }
            })
    ...
    $("#enable-btn").click(function () {
        if (ztree.getSelectedNodes()[0].parentId != null &&
            ...
            swals("这个组织的上级组织处于非正常状态无法启动！！");
        } else {
            swalFunction("你是否要启用这个组织", function () {
                zudp.ajax("/api/group/enable/" + checkId)
                    .get()
                    .then(function (value) {
                        if (value) {
                            $("#enable-btn").hide();
...
function revokeNode() {
    zudp.ajax("/api/group/revoke/" + checkId)
        .get()
        .then(function (value) {
            if (value) {
                $("#enable-btn").show();
...
function batchUpdateGroup_insert(data) {
    zudp.ajax("/api/group/updateUserGroup/" + checkId)
...
function batchUpdateGroup(data) {
    zudp.ajax("/api/group/deleteUserGroup/" + checkId)
        .post(JSON.stringify(data))
        .then(function (da) {
            zudp.plugin.dialog("success").alert("操作成功！", "提示");
            zudp.plugin.table("#list-details").then().ajax.reload();
...
function getNodeDetail(groupId) {
    ...
    zudp.ajax('/api/group/' + groupId).get().then(function (value) {
...
function findList() {
    ...
    zudp.ajax("/api/group/tree").get().then(function (data) {
        debugger;
